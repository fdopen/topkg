...
let lib =
  let drop_exts =
    let c = Topkg_conf_ocaml.v `Host_os in
    let native = Topkg_conf_ocaml.native c in
    let native_dynlink = Topkg_conf_ocaml.native_dynlink c in
...
let bin_drops =
  if not Topkg_conf_ocaml.(native (v `Host_os))
...
let bin_mvs
  ...
  let src, dst =
    ...
    in
    let src =
      if Topkg_conf_ocaml.(native (v `Host_os))
