...
module File = struct
  ...
  let read file =
    try
      let ic = if file = dash then stdin else open_in file in
...
module Cmd = struct
  ...
  let cmdline ?stdout ?stderr cmd =
    if cmd = [] then err_empty_cmd () else
    let cmd = List.rev_map Filename.quote cmd in
  ...
  let exists cmd =
    let bin = try List.(hd (rev cmd)) with Failure _ -> err_empty_cmd () in
  ...
  | false -> R.error_msgf "%s: no such command" (List.hd (List.rev cmd))
  | true -> Ok cmd
  ...
  let out_lines ?trim o =
    out_string ?trim o
    >>= function (s, st) -> Ok (Topkg_string.cuts ~sep:'\n' s, st)
